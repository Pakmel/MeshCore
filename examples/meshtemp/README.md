...
| `MESHTEMP_REGION` | `meshtemp_config.h` | `"se1780"` | Transport-code region scope (see "Region scoping"). Set it to your own local region code, or `""` (empty string) to send unscoped so every repeater forwards it regardless of region configuration. |
