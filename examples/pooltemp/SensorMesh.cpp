...
#include "pooltemp_config.h"   // POOLTEMP_RX_BOOSTED_GAIN
      ...
      telemetry(MAX_PACKET_PAYLOAD - 4)
{
  ...
  // POOLTEMP_RX_BOOSTED_GAIN in pooltemp_config.h.
  _prefs.rx_boosted_gain = POOLTEMP_RX_BOOSTED_GAIN;
...
void SensorMesh::sendSelfAdvertisement(int delay_millis, bool flood) {
  ...
  if (pkt) {
    if (flood) {
      ...
      // Only path this ever takes in PoolTemp: the boot advert (see
